...
%% ===================== PRE-ALLOCATION =====================
num_DF = length(delta_f_vec);   % ← MUST be defined BEFORE parfor

feasible_record              = false(Ns, num_DF);
Convex_min_Rk                = zeros(Ns, num_DF);
Convex_Convergence_curve_AO  = zeros(Ns, num_DF);
Convex_Fake_Convergence_curve_AO = zeros(Ns, num_DF);
Convex_Real_Convergence_curve_AO = zeros(Ns, num_DF);
...
prev_min_Rk   = 0;
    % ================================================================
for n_df_idx = 1:num_DF
delta_f = delta_f_vec(n_df_idx);
...
for k =1:K
    ...
    for q = 1:Q_j
        h_jq(:, q,k) = sqrt(gamrnd(m_q(k,q), omega_q(k,q)/m_q(k,q), [Nr, 1])) .* exp(1i*2*pi*rand(Nr,1));
    end
...
for l=1:nF+L
        ...
        for q = 1:Q_j
            h_jq(:, q,K+l) = sqrt(gamrnd(m_q(K+l,q), omega_q(K+l,q)/m_q(K+l,q), [Nr, 1])) .* exp(1i*2*pi*rand(Nr,1));
        end
...
for ao = 1:max_AO_iter
   ...
   [R_sec_next2,~] = get_Secrecy_matrix(b0, L_node, E_node, alpha, K, nF, sigma2, Pw, AN_P_ratio);
    ...
    fprintf(['AO Iter %2d | Feasible = %d | Fake Sec = %.6f | Δ = %.6f | ' ...
             'max(xi)=%.2e | Δf = %2d | Ns=%2d\n'], ...
            ao, feasible_flag, best_fake_secrecy, ...
            best_fake_secrecy - prev_fake, max(xi_val), delta_f, mc_iter);
...
fprintf('\nConvex AO Finished! Best Fake Secrecy Rate = %.8f\n', best_fake_secrecy);
   % Track feasibility
 feasible_record(mc_iter,n_df_idx) = feasible_flag;

 Convex_min_Rk(mc_iter,n_df_idx) = prev_min_Rk;
 Convex_Convergence_curve_AO(mc_iter,n_df_idx) = prev_cost;
 Convex_Fake_Convergence_curve_AO(mc_iter,n_df_idx) = best_fake_secrecy;
 Convex_Real_Convergence_curve_AO(mc_iter,n_df_idx) = best_real_secrecy;
...
hold on;
plot(delta_f_vec,Convex_Convergence_curve_AO_mean(1:end), 'Color', colors(3,:), 'LineStyle','-.', 'LineWidth',2, 'Marker','o', 'MarkerIndices',1:markerInterval:length(Convex_Convergence_curve_AO_mean), 'MarkerFaceColor',colors(3,:))


title('Convergence Curve','FontWeight','bold','FontSize',12);
xlabel('$\Delta_f$','FontWeight','bold','FontSize',11, 'Interpreter', 'latex');
ylabel('min SC','FontWeight','bold','FontSize',11);
...
plot(delta_f_vec,Convex_Fake_Convergence_curve_AO_mean(1:end), 'Color', colors(3,:), 'LineStyle','--', 'LineWidth',1.5, 'Marker','s', 'MarkerIndices',1:markerInterval:length(Convex_Fake_Convergence_curve_AO(1:end)), 'MarkerFaceColor',colors(3,:));
plot(delta_f_vec,Convex_Real_Convergence_curve_AO_mean(1:end), 'Color', colors(3,:), 'LineStyle','-', 'LineWidth',1.5, 'Marker','^', 'MarkerIndices',1:markerInterval:length(Convex_Real_Convergence_curve_AO(1:end)), 'MarkerFaceColor',colors(3,:));
