function [sc_c_lk,sc_p_lk,sc_p_kk,rate_c_min,rate_p_vec,R_k,sinr_c_k, sinr_p_k, sinr_c_l, sinr_p_l] = compute_sinr_sc_an_noma(Pe,P,Q_j,L,K,delta_f,Plos,PLj,Nr,HB,HA,g_pq,Nsymb,reflect,Rmin,h_rp,h_jq,h_e,zeta_k_St,Active_Gain_dB,AN_P_ratio,x)
    %
    % phi=[];
    % alpha=[];
    alpha = x(1:K);
    ...
    for k = 1:K
        ...
        [Nc_k] = compute_OTFS_static_channel(0,Pe,P,Q_j,Plos(k,1),PLj(k,1),Nr,HB(:,:,:,k),HA(:,:,:,:,k),g_pq(:,:,k),beta_r,Nsymb,h_rp(:,:,1),h_jq(:,:,k,1),h_e(:,k,1),'vectorized'); %Data Channel
