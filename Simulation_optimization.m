...
SP  = population_size; % size of population (number of agents)
MNI = max_iterations; % maximum number of iterations
Rmin=0.01;
...
phi = 2*pi*rand(SP,Nr);
alpha = rand(SP, K+1); 
% divide each row by its row sum to ensure sum(alpha,2)==1.0
alpha = alpha ./ sum(alpha, 2);
% minimize floating point precision error leading to sum(alpha,2)>1.0
...
if any_reflect
    ...
    zeta_k_St = rand(SP,Nr);
    X = [alpha,phi,zeta_k_St];
end

y_optimal = zeros(1, dim); % best solution reached so far

best_sum_rate=0; % sum rate of best solution (our objective function)

population_sum_rate = zeros(SP,1); % sum rate of each agent in population
...
while t<=MNI
    ...
    for i=1:SP
        ...
        population_sum_rate(i) = sum(R_k);
        if population_sum_rate(i)>best_sum_rate
            y_optimal=X(i,:);
            best_sum_rate = population_sum_rate(i);
        end
    end
    ...
    if mod(t,2) == 0
        ...
        for i=1:SP
            ...
            v_i = (v_i<lb).*(2*lb-v_i)+(v_i>=lb).*v_i;
            v_i = (v_i>ub).*(2*ub-v_i)+(v_i<=ub).*v_i;
            ...
            [sc_c_lk,sc_p_lk,sc_p_kk,rate_c,rate_k,R_k,~] = compute_sinr_sc(Pe,P,Q_j,L,K,m_e,m_q,m_p,omega_e,omega_p,omega_q,delta_f,Plos,PLj,Nr,HB,HA,g_pq,Nsymb,reflect,Rmin,h_rp,h_jq,h_e,alpha,phi,zeta_k_St,v_i);
            new_sum_rate = sum(R_k);

            if new_sum_rate > population_sum_rate(i)
                % if new solution doesn't violate constraint and is better
                % than old solution, we replace old solution with new
                % solution
                population_sum_rate(i) = new_sum_rate;
                X(i,:) = v_i;
            end
        end
    else
...
for i=1:size(X,1)
    ...
    % question: if we set penalty=0 then do penalty=penalty+x, isn't it the
    % same thing as penalty=x?
...
while t<=Max_iteration
    ...
    for i=1:size(X,1) % in i-th solution
        ...
        for j=1:size(X,2) % in j-th dimension
            ...
            % question: do r2,r3,r4 parameters update for every i,j pair?
            % pseudocode from the SCA paper updates once every iteration
...
best_PSO = final_sum_rate;

%% display best results for all
display(['Best Sum Rate Hybrid SCA: ', num2str(best_HSCA)]);
display(['Best Sum Rate SCA: ', num2str(best_SCA)]);
display(['Best Sum Rate PSO: ', num2str(best_PSO)]);