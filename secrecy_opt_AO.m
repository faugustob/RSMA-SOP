...
function [phi_St] = optimize_phi_sca_fixed_alpha(alpha, phi_St, phi_Sr, zeta_k_St, ...
        ...
        %% ========================= PRECOMPUTE CHANNELS =========================
        beta_prev = exp(1i*phi_St);
        B_prev = [beta_prev'; 1]*[beta_prev 1];
        ...
        %     1, Pe, P, Q_j, Plos(k,1), PLj(k,1), Nr, ...
        %     HB(:,:,:,k), HA(:,:,:,:,k), g_pq(:,:,k), ...
        %     beta_r, Nsymb, h_rp(:,:,k,1), h_jq(:,:,k), ...
        ...
        for sca_iter = 1:max_sca
            cvx_begin 
                ...
                variable B(Nr+1, Nr+1) complex hermitian   
                ...
                subject to
                   ...
                   % 2. SDR Constraints
                    B == semidefinite(Nr+1); % Must be Positive Semidefinite
                    B(Nr+1, Nr+1) == 1;      % The "1" in the augmented corner
                    diag(B(1:Nr, 1:Nr)) == 1; % Equivalent to abs(beta_r) <= 1
                    ...
                    for k = 1:K    
                       ...
                       V_aug = [V1, V2; V2', term3]*scaling_factor;                    
                       V_AN_aug = [V1_AN, V2_AN; V2_AN', term3_AN]*scaling_factor;  

                       S_f  = (alpha_pi(k) * V_aug);
                       I_f(:,:,k) = ((sum_alpha_pi - alpha_pi(k)) * V_aug+ AN_P_ratio * V_AN_aug + noise*E);
           
        
                        S_j(k) = real(trace(S_f*B));
                        I_j(k) = real(trace(I_f(:,:,k)*B ));
                        I_j_prev(k) = real(trace(I_f(:,:,k)*B_prev));
            ...
            for l = 1:nF
                for k = 1:K
                    ...
                    V_lk_aug = [V1_lk, V2_lk; V2_lk', term3_lk]*scaling_factor;
                    V_AN_lk_aug = [V1_AN_lk, V2_AN_lk; V2_AN_lk', term3_AN_lk]*scaling_factor;
                    ...
                    N_e = (sum(alpha(2:end)) - alpha(k)) * V_lk_aug + AN_P_ratio * V_AN_lk_aug + noise*E;
                    D_e = sum(alpha(2:end)) * V_lk_aug + AN_P_ratio * V_AN_lk_aug + noise*E;
            
                    S_l = real(trace(N_e*B));
                    S_l_prev = real(trace(N_e*B_prev));

                    I_l =  real(trace(D_e*B));

                    % The Taylor expansion (Linear expression in B)
                    lin_neg_log_Ij = -log(I_j_prev(k))/log(2) ...
                    - real(trace(I_f(:,:,k) * (B - B_prev))) / (log(2)*I_j_prev(k));

                    lin_neg_log_Sl = -log(S_l_prev)/log(2) - real(trace(N_e * (B - B_prev))) / (log(2)*S_l_prev);
                            
                 
                    % Legitimate rate is concave (log is handled by CVX)
                                 s_fake(l,k) <= ...
                      log(I_j(k) + S_j(k))/log(2)      ... concave
                    + log(I_l)/log(2)            ... concave
                    + lin_neg_log_Ij              ... affine
                    + lin_neg_log_Sl;             ... affine
            ...
            % 5. Extract beta_r from B (Rank-1 Approximation)
            [V_eig, D_eig] = eig(double(B));
