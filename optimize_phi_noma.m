function [phi_St,cost_opt] = optimize_phi_noma(Rmin,L_node,E_node,problem,b0,alpha,K, nF, sigma2, Pw, AN_P_ratio, Ck)
       ...
       % fprintf('Starting Manifold Optimization...\n');
        ...
        options.maxiter = 5;         % Maximum outer iterations
...
function [grad_Rsec_lk, g_user_all] = grad_R_sec(beta, L_node, E_node, alpha, K, nF, sigma2, Pw, AN_P_ratio)
    ...
    [~,~,~,T_k,T_l,I_k,I_l,Ak,Pk,Al,Pl] = get_Secrecy_matrix_noma(beta, L_node, E_node, alpha, K, nF, sigma2, Pw, AN_P_ratio);
...
function h = hess_func(beta, v, L_node, E_node, alpha, s_param, K, nF, sigma2, Pw, AN_P_ratio, Rmin, lambda_penalty, Ck)
    ...
    [R_sec,rate_p,interf_power_coeff,T_k,T_l,I_k,I_l,Ak,Pk,Al,Pl] = get_Secrecy_matrix_noma(beta, L_node, E_node, alpha, K, nF, sigma2, Pw, AN_P_ratio);
    ...
    for k = 1:K
        ...
        dIk      = interf_power_coeff(k) * dPk + AN_P_ratio * dAk;
        dTk      = alpha(k) * dPk + dIk;
        denom_k  = ln2 * T_k(k) * I_k(k);
        ddenom_k = ln2 * (dTk * I_k(k) + T_k(k) * dIk);
        
        h_user_k = alpha(k) * (du_k * denom_k - u_k * ddenom_k) / (denom_k^2);
        g_user_k = (alpha(k) / denom_k) * u_k;
        ...
        for l = 1:nF
            ...
            dIl      = interf_power_coeff(k) * dPl + AN_P_ratio * dAl;
            dTl      = alpha(k) * dPl + dIl;
            denom_l  = ln2 * T_l(l,k) * I_l(l,k);
            ddenom_l = ln2 * (dTl * I_l(l,k) + T_l(l,k) * dIl);
            
            h_eav_l = alpha(k) * (du_l * denom_l - u_l * ddenom_l) / (denom_l^2);
            g_eav_l = (alpha(k) / denom_l) * u_l;
